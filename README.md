...
### 골든 평가 (v2 골든, 38 케이스 기준) — 합격

| 지표 | 목표 | 세션 시작 | **현재** |
| --- | --- | --- | --- |
| 업무분류 recall | 80% | 100% | **83.3%** |
| 업무흐름 recall | 80% | 90% | **100%** |
| 필수 사용자 여정 | 100% | 50% | **100%** |
| 성공 시나리오 recall | 80% | 25% | **86.4%** (19/22) |
| 복원 recall | 참고 | 9.1% | 43.8% |
| critical gaps | — | 3건 | **0건** |

taxonomy·scenario 양쪽 모두 `passed: true`입니다. 가설의 측정 기준을 처음으로 통과했습니다.
...
2026-09-17~18 세션에서 제품 결함 36건을 고쳤습니다. 전부 회귀 테스트를 동반합니다. 상세는 [보고서](docs/reports/2026-09-18-complete-journey-milestone.md)에 있습니다.

핵심은 **실패의 대부분이 모델 능력이 아니라 계약 쪽 문제였다**는 점이고, 그중에서도 **정보를 주는 것이 거부하는 것보다 효과적이었습니다**.

| 접근 | 횟수 | 결과 |
| --- | --- | --- |
| 정보 제공 | 5 | 모두 즉시 해결 |
| 게이트 추가 | 4 | 모두 충족 불가 계약 발생, 되돌리거나 조건 수정 |

백엔드가 판단에 필요한 사실을 주면 모델이 스스로 맞춥니다. 사실을 주지 않고 거부만 하면 모델은 만족 불가능한 제약 사이를 진동합니다.
...
| 항목 | 내용 |
| --- | --- |
| **업무분류 recall 83.3%** | `AXSE2-BC-002`(TS 작업 관리)가 독립 분류로 묶이지 않고 업로드 분류에 흩어져 있습니다 |
| **성공 시나리오 3건** | 토큰 claim 파싱 실패 대체, 드래그 앤 드롭, 파싱 완료 문서 재업로드. 셋 다 같은 결과에 이르는 대체 조작 경로이고, 현재 컴파일러는 milestone당 하나의 경로만 만듭니다 |
| **복원 recall 43.8%** | 참고 지표이나 9건 미커버 |
...
1. RA-DAR 일반화 확인 — 이번 세션 수정이 AXSE 전용 가정에 기대지 않는지 실증
2. 제품 backend 등록 경로로 동일 결과 재현
3. 대체 조작 경로를 별도 케이스로 도출
4. 업무 분류 세분화
