...
> 최초 작성 이후 골든 평가를 실행하고, 그 결과가 드러낸 결함을 고쳐 다시 측정했다. 경과는 8절, 합격 시점은 9절에 있다.
...
`06-golden-evaluation-signal` 기준. taxonomy와 scenario 양쪽 모두 `passed: true`다.

| 지표 | 목표 | 세션 시작 | 8절 시점 | **합격 시점** |
| --- | --- | --- | --- | --- |
| 업무분류 recall | 80% | 100% | 100% | 83.3% |
| 업무흐름 recall | 80% | 90% | 91.7% | **100%** |
| 필수 사용자 여정 | 100% | 50% | 100% | **100%** |
| 성공 시나리오 recall | 80% | 25% | 54.5% | **86.4%** (19/22) |
| 복원 recall | 참고 | 9.1% | 18.8% | 43.8% |
| critical gaps | — | 3건 | 3건 | **0건** |

### 9-1. 무엇이 합격을 만들었나

게이트가 아니라 정보 제공이다. `businessClassificationSourceSupport`가 영역마다 다음을 함께 보고하도록 했다.

```
journey_action_count: n            스캐너가 판정한 여정 동작 수
journey_action_source_refs: [...]  어느 소스에서 세었는지
```

판정은 백엔드(스캐너)가 소유하고 영역 묶기는 모델이 소유한다. 디렉터리명이나 파일 경로 규칙이 들어가지 않아 프로젝트에 종속되지 않는다. `journey_action_source_refs`를 함께 주는 이유는 모델이 인용한 증거가 좁으면 count가 낮게 나올 수 있어, 숫자를 판결이 아니라 재확인 단서로 읽게 하기 위해서다.

거부 규칙은 하나도 추가하지 않았다. 효과가 연쇄했다.

1. 04가 "설정에서 … 조정하고 저장한다" → "설정 화면에서 … 확인한 뒤 업무 매핑 단계로 이동한다"로 주장 수위를 스스로 낮춤
2. milestone 22 → 17개로 정리
3. 04a 링크와 05 시나리오 품질 상승
4. 골든의 critical gap 3건 소멸

### 9-2. 게이트 판정 조건 수정

`USER_JOURNEY_WORKFLOW_LINK_DEAD_END`가 도착 화면 하나라도 막다르면 거부하고 있었다. 올바른 조건은 전부 막다를 때다. 하나라도 이어지면 여정은 진행할 수 있으므로, 중간에 조회용 화면을 들르는 정당한 milestone까지 막고 있었다. 이 오류가 앞서 관측한 "게이트가 모델을 진동시킨다"의 일부 원인이었다.

### 9-3. 이 세션이 남긴 방법론

| 접근 | 횟수 | 결과 |
| --- | --- | --- |
| 정보 제공 | 5 | 모두 즉시 해결. edge identity(8회 연속 실패 → 소멸), 유효 스레드 목록(가짜 이름 6회 → 소멸), 어긋난 필드명, 미해결 milestone 목록, 여정 동작 수 |
| 게이트 추가 | 4 | 모두 충족 불가 계약 발생, 되돌리거나 조건 수정 필요 |

백엔드가 판단에 필요한 사실을 주면 모델이 스스로 맞춘다. 사실을 주지 않고 거부만 하면 모델은 만족 불가능한 제약 사이를 진동한다.

### 9-4. 남은 격차

| 항목 | 내용 |
| --- | --- |
| 업무분류 recall 83.3% | `AXSE2-BC-002`(TS 작업 관리) 미매칭. 후보가 작업 선택·생성을 업로드 분류에 흩어놓고 목록 조회·자동 진입·전환을 독립 분류로 묶지 않았다 |
| 성공 시나리오 3건 | `SCN-005` 토큰 claim 파싱 실패 대체, `SCN-016` 드래그 앤 드롭, `SCN-019` 파싱 완료 문서 재업로드. 셋 다 같은 결과에 이르는 대체 조작 경로이고, 현재 컴파일러는 milestone당 하나의 경로만 만든다 |
| 복원 recall 43.8% | 참고 지표. 9건 미커버 |
| 제품 backend 등록 | 미시도. 전 산출물이 로컬 probe |
| RA-DAR 일반화 | 미검증. 이번 세션 수정이 AXSE 전용 가정에 기대지 않는지 확인이 필요하다 |
